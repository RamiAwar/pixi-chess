...
function setupPieces(chess_controller, squareSize, pieceContainer){
    ...
    for	(let i = 0; i < board.length; i++) {
        for (let j = 0; j < board[i].length; j++) {
            ...
            if(piece != null){
                createPieceSprite(piece, i, j, squareSize, pieceContainer);
            }
        }
    }
}

function createPieceSprite(piece, rank, file, squareSize, pieceContainer){
    let pieceIndex = PIECE_ORDER.indexOf(piece.type);
    let texture = pieceTextures[pieceIndex][(piece.color == 'b')];
    let sprite = new PIXI.Sprite(texture);
    let pos = new Point(file*squareSize+squareSize*.5,rank*squareSize+squareSize*.5);

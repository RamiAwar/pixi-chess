...
function createPieceSprite(chess_controller, piece, rank, file, squareSize, pieceContainer, pieceTextures){
    ...
    sprite.rank = isBlack ? rank : 7 - rank;
    sprite.file = file;
    ...
    sprite.release = () => {
        if(sprite.isDragged){
            ...
            toSquareName = FILES[closest_square.x] + RANKS[closest_square.y];
            pieceName = sprite.piece != 'p' ? sprite.piece : '';
            
            move = chess_controller.move({
                from: pieceName + fromSquareName,
                to: pieceName + toSquareName
            });
            clg({
                from: pieceName + fromSquareName,
                to: pieceName + toSquareName
            ...
            if(move === null){
                ...
                coords = positionToCoord(sprite.file, sprite.rank, squareSize);
                sprite.position.set(coords.x, coords.y);
            }else{
                sprite.rank = closest_square.y;
...
function findClosestSquare(position){
    return new Point(
        ...
        7 - Math.floor(position.y/squareSize)
    )
}

function positionToCoord(i, j, squareSize){
    return new Point(i * squareSize + squareSize * 0.5, (7 - j) * squareSize + squareSize * 0.5)
}
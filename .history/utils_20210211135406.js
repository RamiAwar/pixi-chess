...
function createPieceSprite(chess_controller, piece, rank, file, squareSize, pieceContainer, pieceTextures){
    ...
    sprite.release = () => {
        if(sprite.isDragged){
            ...
            clg(move)
            clg(chess_controller.turn())
            // Discard if not turn or move invalid
            if(move === null || (chess_controller.turn() == 'w' && sprite.isBlack) || (chess_controller.turn() == 'b' && !sprite.isBlack)){
